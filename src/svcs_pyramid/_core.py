...
def init(
    ...
    *svcs* uses a :term:`tween` to manage the life cycle of the container. You
    ...
    config.add_tween(
        "svcs_pyramid.ServicesTween", over=tween_over, under=tween_under
    )
...
def get_abstract(request: Request, *svc_types: _ServiceType) -> Any:
    ...
    *request*.
    ...
    .. deprecated:: 26.1.0
       Thanks to :pep:`747`, :func:`get()` can be used instead.
    """
...
def get(request: Request, *svc_types: _ServiceType) -> object:
    """
    Same as :meth:`svcs.Container.get()`, but uses the container from
    *request*.
    """
    return svcs_from(request).get(*svc_types)
